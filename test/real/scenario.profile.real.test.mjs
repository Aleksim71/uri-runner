// path: test/real/scenario.profile.real.test.mjs
...
describe('real profile: scenario', () => {
  ...
  it('scenario.classification_required.unknown_named', async () => {
    ...
    const outbox = readJson(outboxJsonPath);
    assert.equal(outbox.status, 'classification_required', 'final outbox status must be classification_required');
    assert.equal(outbox.error?.code, 'CLASSIFICATION_REQUIRED', 'error.code mismatch');
    assert.ok(outbox.classification_request && typeof outbox.classification_request === 'object', 'classification_request must be present in outbox.json');
    assert.equal(outbox.classification_request.engine, 'scenario', 'classification_request.engine mismatch');
    assert.ok(Array.isArray(outbox.classification_request.unknown_steps), 'classification_request.unknown_steps must be an array');
    assert.equal(outbox.classification_request.unknown_steps.length, 1, 'expected exactly one unknown scenario step');
